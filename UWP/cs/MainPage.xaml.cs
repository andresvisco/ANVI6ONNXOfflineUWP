...
namespace SqueezeNetObjectDetection
{
    ...
    {
        private const string _kModelFileName = "viscoreitano.onnx";//"ccb4c66f4d3243b9a375ee34a256f54b.onnx";
        ...
        private async Task EvaluateVideoFrameAsync(VideoFrame inputFrame)
        {
            if (inputFrame != null)
            {
                try
                {
                    ...
                    viscoreitanoInput viscoreitanoInput = new viscoreitanoInput();
                    ImageFeatureValue imageFeatureValue = ImageFeatureValue.CreateFromVideoFrame(inputFrame);
                    LearningModelBinding binding = new LearningModelBinding(_session);

                    viscoreitanoInput.data = imageFeatureValue;
                    binding.Bind("data", imageFeatureValue);

                    var resultado =await _session.EvaluateAsync(binding, $"Run {++_runCount}");
                    var valor = resultado.Succeeded.ToString();
                    ...
                    //var resultTensor = results.Outputs["classLabel"] as TensorFloat;
                    ...
                    string resultadoVector = string.Empty;
                    ...
                    for (int i = 0; i < resultVector.Count(); i++)
                    {
                        ...
                        for (int j = 0; j < 3; j++)
                        {
                            resultadoVector = resultVector[i].ToString();
                    ...
                    StatusBlock.Text = resultadoVector.ToString();
                    // Display the result
